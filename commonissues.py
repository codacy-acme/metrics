...
def getDetailsForCommit(baseurl, provider, organization, repository, commitsha, token):
    if token == None:
    ...
    hasNextPage = True
    while hasNextPage:
        url = '%s/api/v3/analysis/organizations/%s/%s/repositories/%s/commits/%s/deltaIssues?limit=100&%s' % (
            baseurl, provider, organization, repository, commitsha, cursor)
        r = requests.get(url, headers=headers)
        response = json.loads(r.text)
        data = response['data']
        for issue in data:
            ...
            if issue['deltaType'] == 'Added':
                delta['added'].append(i)
            else:
                delta['fixed'].append(i)
        hasNextPage = 'cursor' in response['pagination']
        if hasNextPage:
            cursor = 'cursor=%s' % response['pagination']['cursor']
    return delta
...
def main():
    parser = argparse.ArgumentParser(description='Codacy Common Issues')
    ...
    if(args.repository == None):
        ...
        for repo in repos:
            ...
            for commit in tmpc:
                commit['delta'] = getDetailsForCommit(
